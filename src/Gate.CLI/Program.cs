...
var rootCommand = new RootCommand("Gate - 代理配置管理工具");
...
envCommand.SetHandler(async (string? http, string? https, string? noProxy, bool clear, bool verify) =>
{
    ...
    if (verify && !string.IsNullOrEmpty(http))
    {
        ...
        {
            Console.WriteLine($"[FAIL] 代理测试失败: {testResult.ErrorMessage}");
            Console.WriteLine("使用 --no-verify 跳过测试");
            return;
        }
        Console.WriteLine($"[OK] 代理测试成功，响应时间: {testResult.ResponseTimeMs}ms");
    }
    
    if (!string.IsNullOrEmpty(http) || !string.IsNullOrEmpty(https))
    {
        EnvVarManager.SetProxyForCurrentProcess(config2);
        Console.WriteLine($"[OK] 已设置代理: {config2}");
    }
    else
    {
        // 显示当前配置
...
toolCommand.SetHandler(async (string? name, string? proxy, bool clear, bool list) =>
{
    if (list)
    {
        ...
        foreach (var cat in ToolRegistry.GetCategories())
        {
            ...
            foreach (var tool in ToolRegistry.GetByCategory(cat))
            {
                ...
                var configStatus = current != null && !current.IsEmpty ? "已配置" : "未配置";
    ...
    if (clear)
    {
        if (targetTool.ClearProxy())
            Console.WriteLine($"[OK] 已清除 {name} 的代理配置");
        else
            Console.WriteLine($"[ERR] 清除 {name} 代理失败");
        return;
    }
    ...
    if (!string.IsNullOrEmpty(proxy))
    {
        if (targetTool.SetProxy(proxy))
            Console.WriteLine($"[OK] 已设置 {name} 代理: {proxy}");
        else
            Console.WriteLine($"[ERR] 设置 {name} 代理失败");
        return;
    }
...
profileCommand.SetHandler((string? name, bool save, bool load, bool delete, bool list, bool setDefault) =>
{
    ...
    if (save)
    {
        ...
        Console.WriteLine($"[OK] 已保存配置集: {name}");
        return;
    }
    ...
    if (load)
    {
        ...
        Console.WriteLine($"[OK] 已加载配置集: {name}");
    ...
    if (delete)
    {
        if (ProfileManager.Delete(name))
            Console.WriteLine($"[OK] 已删除配置集: {name}");
        else
            Console.WriteLine($"[ERR] 删除配置集失败: {name}");
        return;
    }
    ...
    if (setDefault)
    {
        ...
        Console.WriteLine($"[OK] 已设默认配置集: {name}");
    }
...
testCommand.SetHandler(async (string? proxy, string? url) =>
{
    ...
    if (result.Success)
        Console.WriteLine($"\n[OK] 连接成功! 响应时间: {result.ResponseTimeMs}ms");
    else
        Console.WriteLine($"\n[FAIL] 连接失败: {result.ErrorMessage}");
}, testProxy, testUrl);

rootCommand.AddCommand(testCommand);

// 汇总查看命令：显示所有当前代理配置
var statusCommand = new Command("status", "查看当前所有代理相关配置概览");

statusCommand.SetHandler(() =>
{
    Console.WriteLine("=== 环境变量代理（用户级） ===");
    var env = EnvVarManager.GetProxyConfig(EnvLevel.User);
    Console.WriteLine($"  HTTP_PROXY : {env.HttpProxy ?? "(未设置)"}");
    Console.WriteLine($"  HTTPS_PROXY: {env.HttpsProxy ?? "(未设置)"}");
    Console.WriteLine($"  NO_PROXY   : {env.NoProxy ?? "(未设置)"}");
    Console.WriteLine();

    Console.WriteLine("=== 已配置的工具代理 ===");
    var anyTool = false;
    foreach (var tool in ToolRegistry.GetAllTools())
    {
        ...
        {
            anyTool = true;
            var installed = tool.IsInstalled() ? "[INST]" : "[UNKNOWN]";
            Console.WriteLine($"  {tool.ToolName,-20} {installed}  {cfg}");
        }
    }
    if (!anyTool)
    {
        Console.WriteLine("  (无工具代理配置)");
    }
    Console.WriteLine();

    Console.WriteLine("=== 配置集（profiles） ===");
    var profiles = ProfileManager.List();
    if (profiles.Count == 0)
    {
        Console.WriteLine("  (无保存的配置集)");
    }
    else
    {
        foreach (var p in profiles)
